...
class StorageMethods {
  ...
  Future<String> uploadDocumnetToStorage(
      File documentfile, String filename) async {
    // mention try catch later on
  ...
  void uploadDocument({
    ...
    @required String receiverId,
    @required String senderId,
    @required DocumentUploadProvider documentUploadProvider,
  }) async {
    final ChatMethods chatMethods = ChatMethods();

    // Set some loading value to db and show it to user
    documentUploadProvider.setToLoading();

    // Get url from the image bucket
    String url = await uploadDocumnetToStorage(document, filename);

    // Hide loading
    documentUploadProvider.setToIdle();

    chatMethods.setDocumentMsg(url, filename, receiverId, senderId);
  }
}
